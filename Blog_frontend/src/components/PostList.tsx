    ...
    <div className="w-full space-y-6">
      ...
      {loading ? (
        <div className="space-y-4">
          {[...Array(3)].map((_, index) => (
            <Card key={index} className="w-full theme-card">
              <CardBody>
                <div className="h-4 skeleton-light w-3/4 mb-4"></div>
                <div className="h-4 skeleton-light w-1/2"></div>
              </CardBody>
            </Card>
          ))}
        </div>
            ...
            {posts?.map((post) => (
              <Card key={post.id} className="w-full p-2 theme-card cursor-pointer transition-all duration-300 hover:scale-[1.01]" isPressable={true} onPress={() => navToPostPage(post)}>
                <CardHeader className="flex gap-3">                 
                    ...
                    <p className="text-small text-theme-secondary">
                      by {post.author?.name}
                    </p>                
                    </div>
                </CardHeader>
                <CardBody>
                  <p className="line-clamp-3 text-theme-secondary">
                    {createExcerpt(post.content)}
                  </p>
                </CardBody>
                <CardFooter className="flex flex-wrap gap-3">
                  <div className="flex items-center gap-1 text-small text-theme-muted">
                    <Calendar size={16} />
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Chip
                      className="bg-theme-chip-accent font-medium"
                    >
                      {post.category.name}
                    </Chip>
                    {post.tags.map((tag) => (
