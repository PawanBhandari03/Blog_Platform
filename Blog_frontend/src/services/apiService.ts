...
class ApiService {
  ...
  public async login(credentials: LoginRequest): Promise<AuthResponse> {
    const payload = {
      email: typeof credentials.email === 'object' && credentials.email !== null ? (credentials.email as any).value || '' : String(credentials.email || ''),
      password: typeof credentials.password === 'object' && credentials.password !== null ? (credentials.password as any).value || '' : String(credentials.password || '')
    };
    
    console.log("Login payload:", payload);
    
    const response: AxiosResponse<AuthResponse> = await this.api.post('/auth/login', payload);
